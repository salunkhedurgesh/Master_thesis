...
%2. At the end of execution the output will be:
 ...
 ranges = [1,2;0.1,0.5;0,0.25;0.5,1];
 n = 4;
%  a_range = [0.25, 1.25];
...
%  t_range = [1,4];
 ...
 %Check for Initial Simplex formation
 [sobol_set, suc_var] = iniSimplex(n, starts, sobol_var, ranges);
 if suc_var == 1
 ...
 %Switch for considering the parameter weightage
     if input(prompt_para) == 0
         [Best_point_yet, ball_halfwidth] = Nelder_mead_actuator_loop(sobol_set,steps,starts,deep_file_name,points_file_name,ranges);
     else
         [Best_point_yet, ball_halfwidth] = Nelder_mead_para_weight(sobol_set,steps,starts,deep_file_name,points_file_name,ranges);
     end
     ...
     [Best_point_yet, Best_rho] = Nelder_mead_refine(Best_point_yet,refine_file_name);
     ...
     fprintf('ERROR: The code cannot calculate the necessary valid simplex points to initialise the optimisation');
     fprintf('\t \tConsider:\n');
     fprintf('\t \t1: Checking the ranges for non-singularity\n');
     fprintf('\t \t2: Provide a bigger range of parameters(May not be useful)\n');
     error('CODE STOPPED DUE TO ABOVE REASONS')
 end
