...
function [Best_point_yet, ball_halfwidth] = Nelder_mead_para_weight(n, sobol_var,steps,starts,deep_file_name,points_file_name,ranges)
    ...
    a_range(1,:) = ranges(1,:);
    a_prime_range(1,:) = ranges(2,:);
    h_range(1,:) = ranges(3,:);
    t_range(1,:) = ranges(4,:);
    offset_range(1,:) = ranges(5,:);
    ...
    fprintf('The range for the offset is %f and %f \n \n', offset_range(1), offset_range(2));
    ...
    fprintf(fileID,'The range for the offset is %f and %f \n \n', offset_range(1), offset_range(2));

    %50 valid points
    valid_points = (n+1)*starts; %Number of starts = valid_points/(n+1)
    optimum = 0;
    ...
    if sobol_var == 0
        sobol_set = rand_tuning(n,a_range,a_prime_range,h_range,t_range,offset_range,valid_points);
    else
         sobol_set = sobol_tuning(n,a_range,a_prime_range,h_range,t_range,offset_range,valid_points);
    end
    ...
    multi_start_evaluations = zeros(1,starts);
    saved_S_eval = zeros(starts, n+1);
    ...
    for multi_start = 1:starts
        ...
        while(iteration <= steps) % stops if the solution has not changed for 10 iterations
            ...
            for i = 1:n+1
                [evaluations(i),~] = obj_func_para(S(i,1), S(i,2), S(i,3), S(i,4), S(i,5), ball_halfwidth);
            ...
            reflect_point = range_respect(reflect_point,ranges,n);
            [reflect_evaluation,~] = obj_func_para(reflect_point(1), reflect_point(2), reflect_point(3), reflect_point(4), reflect_point(5), ball_halfwidth);
            ...
            elseif reflect_evaluation < min_eval
                ...
                expand_point = range_respect(expand_point,ranges,n);
                [expand_evaluation, ~] = obj_func_para(expand_point(1), expand_point(2), expand_point(3), expand_point(4), expand_point(5),ball_halfwidth);
            ...
            elseif reflect_evaluation > second_max_eval
                ...
                if second_max_eval < reflect_evaluation && reflect_evaluation < max_eval
                    ...
                    outside_contract_point = range_respect(outside_contract_point,ranges,n);
                    [outside_contract_evaluation,~] = obj_func_para(outside_contract_point(1), outside_contract_point(2), outside_contract_point(3), outside_contract_point(4), outside_contract_point(5), ball_halfwidth);
                    ...
                    if outside_contract_evaluation < reflect_evaluation
                        ...
                        for i = 2:n+1
                          ...
                          Sort_S(i,:) = range_respect(Sort_S(i,:),ranges,n);
                        end
                    end

                elseif reflect_evaluation > max_eval
                    inside_contract_point = mean_point - k_coeff*(mean_point-Sort_S(n+1,:));
                    inside_contract_point = range_respect(inside_contract_point,ranges,n);
                    [inside_contract_evaluation,~] = obj_func_para(inside_contract_point(1), inside_contract_point(2), inside_contract_point(3), inside_contract_point(4), inside_contract_point(5), ball_halfwidth);
         ...
         [~,rho_vec]= obj_func_para(S_min(1), S_min(2), S_min(3), S_min(4), S_min(5), ball_halfwidth);
         rho1_min = rho_vec(1);
         rho1_max = rho_vec(2);
         rho2_min = rho_vec(3);
         rho2_max = rho_vec(4);
        ...
        if optimum == 1
            ...
            fprintf(fileID, "a = %f, a_prime = %f, h = %f t = %f and offset = %f \n", S_min(1), S_min(2), S_min(3), S_min(4), S_min(5));
        ...
        fprintf("Best point yet is [%f,%f,%f,%f,%f] after %d iterations \n", Best_point_yet(1),Best_point_yet(2),Best_point_yet(3),Best_point_yet(4), Best_point_yet(5), multi_start);
        fprintf(fileID, "Best point yet is [%f,%f,%f,%f,%f] after %d iterations \n", Best_point_yet(1),Best_point_yet(2),Best_point_yet(3),Best_point_yet(4), Best_point_yet(5), multi_start);
    ...
    for i = 1:size(saved_S_eval,1)
      fprintf(fileID2, "[%f,%f,%f,%f,%f],{%d} \n \n", saved_S_eval(i,1),saved_S_eval(i,2),saved_S_eval(i,3),saved_S_eval(i,4), saved_S_eval(i,5), saved_S_eval(i,6));
    end
    fclose(fileID2);
