...
function [S_min, rho_vec] = Nelder_mead_refine(n, S_raw,refine_file_name, ranges)

    fprintf('Opening file named refine_report.text for storing the optimised points after each start \n');
    fileID = fopen(refine_file_name,'a');
    sphere_limit = 45;
    ...
    S = zeros(n+1,n);
        ...
        offset_stroke = offset_range(2) - offset_range(1);
        percent_chart = [-1, 1, -1, -1, 1; 1, 3, 1, -2, -1; -2, 0, -3, -5, 1; 1, -1, 2, 2, 1;1, 1, -1, -2, 1];
        ...
        for i = 1:n
            S(i+1,:) = [S_raw(1)+(a_stroke*percent_chart(i,1))*0.01, S_raw(2)+(a_prime_stroke*percent_chart(i,2))*0.01, S_raw(3)+(h_stroke*percent_chart(i,3))*0.01, S_raw(4)+(t_stroke*percent_chart(i,4))*0.01, S_raw(5)+(offset_stroke*percent_chart(i,5))*0.01];
        end
