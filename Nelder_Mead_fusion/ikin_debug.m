function [rho, p_lim_u1, p_lim_s1, p_lim_s2] = ikin_debug(in_origin_t, in_origin_u, in_origin_s, in_origin_s_initial, in_t_s12_initial, limits, alpha, beta)
...
%         if alpha == 0 || beta == 0
%             s12_align_torsion_axis
%             [norm(abs(s12_align_torsion_axis)/norm(s12_align_torsion_axis) - [1; 0; 0]), alpha, beta]
%             norm(abs(s12_align_torsion_axis)/norm(s12_align_torsion_axis) - [1; 0; 0]) > 0.00001
%         end
        ...
        s_joint_z = s12_tilt_mat * in_origin_z;
        ...
        s_joint_z = s12_tilt_mat * s12_torsion_mat * in_origin_z;
        if abs(s_joint_x(2)) > sind(limits(2)) || abs(s_joint_z(2)) > sind(limits(2))
            p_lim_s2 = 0;
        else
