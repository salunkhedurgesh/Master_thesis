...
clear all;
clc
fileID = fopen('records_to_supervisors.txt','a');
...
valid_points = 500;
sobol_var = true;
sobol_set = sobol_tuning(n,a_range,a_prime_range,h_range,t_range,valid_points,sobol_var);
...
if act_trigger == 0
    fprintf("ACTUATOR CONSTRAINTS ARE RELAXED (change act_trigger variable = 1 to switch) \n");
    fprintf(fileID,"ACTUATOR CONSTRAINTS ARE RELAXED \n");
else
    fprintf("ACTUATOR CONSTRAINTS ARE ACTIVATED (change act_trigger variable = 0 to switch) \n");
    fprintf(fileID,"ACTUATOR CONSTRAINTS ARE ACTIVATED \n");
end
...
for i = 1:5
    [evaluations(i),~] = obj_func(S(i,1), S(i,2), S(i,3), S(i,4),act_trigger);  
...
[reflect_evaluation,~] = obj_func(reflect_point(1), reflect_point(2), reflect_point(3), reflect_point(4),act_trigger);
...
elseif reflect_evaluation < min_eval
    ...
    [expand_evaluation, ~] = obj_func(expand_point(1), expand_point(2), expand_point(3), expand_point(4),act_trigger);
...
elseif reflect_evaluation > second_max_eval
    if second_max_eval < reflect_evaluation && reflect_evaluation < max_eval
        ...
        [outside_contract_evaluation,~] = obj_func(outside_contract_point(1), outside_contract_point(2), outside_contract_point(3), outside_contract_point(4),act_trigger);
    ...
    elseif reflect_evaluation > max_eval
        ...
        [inside_contract_evaluation,~] = obj_func(inside_contract_point(1), inside_contract_point(2), inside_contract_point(3), inside_contract_point(4),act_trigger);
...
[~,rho_vec]= obj_func(S_min(1), S_min(2), S_min(3), S_min(4),act_trigger);
rho1_min = min(rho_vec(:,1));
rho1_max = max(rho_vec(:,1));
rho2_min = min(rho_vec(:,2));
rho2_max = max(rho_vec(:,2));
...
fclose(fileID);
plot_limit(Best_point_yet(1), Best_point_yet(2), Best_point_yet(3), Best_point_yet(4),act_trigger);

fileID2 = fopen('Pointsrecords_25march_1.txt','a');
