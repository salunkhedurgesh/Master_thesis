...
function [Best_point_yet, ball_halfwidth,S] = Nelder_mead_para_weight(sobol_var,steps,starts)

    fprintf('Opening file named Report.text for storing the optimised points after each start \n');
    fileID = fopen('Report_para_5_April2.txt','a');
    sphere_limit = 45;
    ...
    for multi_start = 1:starts
        ...
        while(iteration <= steps) % stops if the solution has not changed for 10 iterations
            ...
            for i = 1:5
                [evaluations(i),~] = obj_func_para(S(i,1), S(i,2), S(i,3), S(i,4),ball_halfwidth);
            ...
            [reflect_evaluation,~] = obj_func_para(reflect_point(1), reflect_point(2), reflect_point(3), reflect_point(4),ball_halfwidth);
            ...
            elseif reflect_evaluation < min_eval
                ...
                [expand_evaluation, ~] = obj_func_para(expand_point(1), expand_point(2), expand_point(3), expand_point(4),ball_halfwidth);
            ...
            elseif reflect_evaluation > second_max_eval
                ...
                if second_max_eval < reflect_evaluation && reflect_evaluation < max_eval
                    ...
                    [outside_contract_evaluation,~] = obj_func_para(outside_contract_point(1), outside_contract_point(2), outside_contract_point(3), outside_contract_point(4),ball_halfwidth);
                ...
                elseif reflect_evaluation > max_eval
                    ...
                    [inside_contract_evaluation,~] = obj_func_para(inside_contract_point(1), inside_contract_point(2), inside_contract_point(3), inside_contract_point(4),ball_halfwidth);
         ...
         [~,rho_vec]= obj_func_para(S_min(1), S_min(2), S_min(3), S_min(4),ball_halfwidth);
         rho1_min = rho_vec(1);
         rho1_max = rho_vec(2);
         rho2_min = rho_vec(3);
         rho2_max = rho_vec(4);
    ...
    fileID2 = fopen('Points_record_para_5_April2.txt','a');
    ...
    for i = 1:size(saved_S_eval,1)
      fprintf(fileID, "[%f,%f,%f,%f],{%d}, (%f %%) \n \n", saved_S_eval(i,1),saved_S_eval(i,2),saved_S_eval(i,3),saved_S_eval(i,4), saved_S_eval(i,5), (saved_S_eval(i,5))/404);
    end
    fclose(fileID2);
