function passive_limit_universal = p_lim_uni(a,a_prime, h, t, ball_halfwidth)
    hk = 0;
    ...
    else if (abs(rot_x_vec(2)) > ball_halfwidth)
            passive_limit_universal(1) = 0;
        end
    else
    ...
    second_tilt_angle = arccos((-cos(alpha)*sin(beta)*hk+sin(alpha)*a_prime+cos(alpha)*cos(beta)*h+t)/sqrt(2*cos(beta)*sin(alpha)*a*h+2*cos(beta)*cos(alpha)*h*t-2*sin(beta)*sin(alpha)*a*hk-2*sin(beta)*cos(alpha)*hk*t+2*sin(alpha)*a_prime*t-2*cos(alpha)*a*a_prime+a^2+a_prime^2+h^2+hk^2+t^2));
    ...
    else if (abs(rot_y_vec(1)) > ball_halfwidth)
            passive_limit_universal(2) = 0;
        end
    else
